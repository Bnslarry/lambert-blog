...
http.createServer(async (req, res) => {
    ...
    if (req.method === 'POST' && req.url === '/') {
      ...
      const projectDir = path.resolve(`./${data.repository.name}-develop`)
     ...
     execSync(`docker build . -t ${data.repository.name}-image:latest `,{
      ...
      // 创建 docker 容器
      execSync(`docker run -d -p 6001:80 --name ${data.repository.name}-container  ${data.repository.name}-image:latest`, {
       stdio:'inherit',
  })
